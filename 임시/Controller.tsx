...
export class Controller{
    ...
    //대출이 가능한지 확인
    protected async lentpossible(mem_no: number): Promise<boolean>{
        ...
        data = await PDO(this.selectID(mem_no), '');
        ...
        data = await PDO(this.whereSQL(where), '');
        result = JSON.parse(JSON.stringify(data));
    ...
    protected async existMat(mat_no: number, mat_exist: number){
        ...
        if(mat_exist == 1){
            ...
            this.table = this.lenTable[0];
            this.keyField = this.lenTable[1];
            var data = await PDO(this.whereSQL(where), '');
    ...
    protected async reservationCheck(res_no: string, mat_no: string, mem_no: string): Promise<boolean>{
        var num: number;
        var where: string;
        var row: string[];
        ...
        if(res_no == null){
            ...
            data = await PDO(this.selectID(+res_no), '');
            row = JSON.parse(JSON.stringify(data));
