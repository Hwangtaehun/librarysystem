...
class Common{
	...
	protected function pagemanager(int $total_cnt, string $value){
        $pagenum = 19;
        $outStr= '';
    ...
    protected function pagesql(string $where){
        if(isset($_GET['page'])){
            $page = $_GET['page'];
        }
        else{
    ...
    protected function lentpossible($mem_no, $memTable, $lenTable){
    ...
    protected function existMat($mat_no, $mat_exist, $lenTable, $delTable, $matTable){
        $bool = true;
    ...
    protected function reservationCheck($res_no, $mat_no, $resTable){
        $rs = false;
